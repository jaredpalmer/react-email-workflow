...
class Copy extends Component {
  ...
  componentDidMount () {
    const { id } = this.props
    new Clipboard(`#${id}`) // eslint-disable-line
  }
      ...
      <div
        ...
        <Flex
          ...
          color={this.state.hovered ? '#ffffff' : L.blue}>
            {!hasCopied ? <span><i className='ion ion-ios-copy' style={{ marginRight: '.5rem' }} /> Copy </span>
              : <span><i className='ion ion-ios-copy-outline' style={{ marginRight: '.5rem' }} /> Copied!</span>}
        </Flex>
      </div>
    )
  }
