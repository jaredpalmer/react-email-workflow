    ...
    <div className="h-full flex flex-col relative">
          ...
          <div className="flex gap-2">
            ...
            <TooltipProvider>
              <Tooltip>
                  ...
                  >
                    <PopoverTrigger asChild>
                      <Button
                        variant="outline"
                        size="icon"
                        className={cn(!meta.date && 'text-muted-foreground')}
                      >
                ...
                <TooltipContent>
                  <p>
                    {meta.date ? `Date: ${meta.date}` : 'Select email date'}
                  </p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-background border-t px-8 py-4 z-20">
        <div className="grid grid-cols-3 gap-3 max-w-md mx-auto">
          <Button onClick={() => handleAddElement('html')}>
            <Code className="h-4 w-4" />
            <span>HTML</span>
          </Button>
          <Button onClick={() => handleAddElement('url')}>
            <Link className="h-4 w-4" />
            <span>Story</span>
          </Button>

          <Button onClick={() => handleAddElement('markdown')}>
